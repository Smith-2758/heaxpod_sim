param(
    ...
    [string]$AssetsDir = (Join-Path $PSScriptRoot '..\..\log\step\compare_user_step_repeat3_20260401_fixbaseline\midterm_assets')
)
...
$markdownFile = 'section_1_2_2_step_revised.md'
$sectionPrefix = '1.2.2 '
$nextSectionPrefix = '1.2.3 '
$imageWidthEmu = 5233973

$figures = @(
    @{
        Placeholder = '图X'
        File = 'figure1_step_trajectory_comparison.png'
        FigureRef = '图5'
        Caption = '图5 高台场景下传统方案与当前方案的质心轨迹对比图'
    },
    @{
        Placeholder = '图Y'
        File = 'figure2_step_key_metrics.png'
        FigureRef = '图6'
        Caption = '图6 高台场景下传统方案与当前方案关键指标对比图'
    }
)
...
function Load-SectionContent {
    ...
    $heading = $blocks[0].Replace("`r", ' ').Replace("`n", ' ').Trim()
    $items = New-Object System.Collections.ArrayList

    foreach ($block in $blocks[1..($blocks.Count - 1)]) {
        $normalized = $block.Replace("`r", "`n").Replace("`n", ' ').Trim()
        $matchedFigure = $null
...
Write-Output 'Updated section 1.2.2 successfully.'
